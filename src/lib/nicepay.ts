import { nicepayClient } from "nodejs-nicepay";

const privateKeyStr = `-----BEGIN PRIVATE KEY-----

  -----END PRIVATE KEY-----`;
const registrationEndPoint = "https://dev.nicepay.co.id/nicepay/redirect/v2/registration";

const client = nicepayClient.SecondApiVersion({
    isProduction: false,
    privateKey: privateKeyStr,
    clientId: "IONPAYTEST",
    clientSecret:
        "33F49GnCMS1mFYlGXisbUDzVf2ATWCl9k3R++d5hDd3Frmuos/XLx8XhXpe+LDYAbpGKZYSwtlyyLOtS/8aD7A==",
    isCloudServer: false
});

const parameter = {
    payMethod: "08",
    currency: "IDR",
};
